...
function roundView(model) {
    ...
    if (model["level"] > 0) {
        if (titleTop === "BOT")
            playerTop += ' level ' + model['level'];
        if (titleBottom === "BOT")
            playerBottom += ' level ' + model['level'];
    }
    ;
    clock_1.renderTimeago();
