...
function register(meta: CataloguedVariantClientDocument) {
    if (!variantNames.includes(meta.name)) variantNames.push(meta.name);
...
test.each([
    ...
    const name = `undroppable-pocket-${count ?? 'default'}`;
    const startFen = 'rnbpk3/8/8/8/8/8/8/RNBPK3[-] w - - 0 1';
    const fsfVariantInfo = makeFsfVariantInfo({
        name,
        board: { startFen },
        pieces: [
        ...
        pieceTypes: ['king', 'pawn', 'knight', 'bishop', 'rook'],
        drops: {
            enabled: true,
            capturesToHand: true,
            noDoubledType: count === undefined ? '' : 'pawn',
            noDoubledCount: count === undefined ? 1 : Number(count),
        },
    });
    const variant = register({
        name,
...
test('catalogued variants preserve royal roles explicitly present in the initial pocket', () => {
    const name = 'explicit-royal-pocket';
    const startFen = '8/8/8/8/8/8/8/8[KQkq] w - - 0 1';
    const fsfVariantInfo = makeFsfVariantInfo({
        name,
        board: { startFen },
        pieces: [fsfPiece('king', 'K'), fsfPiece('queen', 'Q')],
        pieceTypes: ['king', 'queen'],
        drops: { enabled: true, capturesToHand: false },
    });
    const variant = register({
        name,
