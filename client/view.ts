...
export function sliderFromList(settings: Settings<number>, name: string, text: string, listId: string, dataList: number[]) {
    const id = name;
    return [
        h('label', { attrs: { for: id } }, text),
        h(`input#${id}.slider`, {
            props: { name: name, type: "range", min: 0, max: dataList.length-1, value: dataList.indexOf(settings.value) },
            attrs: { list: listId },
