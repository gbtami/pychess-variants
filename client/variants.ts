...
function cataloguedPocketRows(meta: CataloguedVariantClientDocument): Record<cg.Color, cg.Letter[]> {
    ...
    for (const color of ['white', 'black'] as const) {
        ...
        const noDoubledRole = typeRoles[color].get(fsf.drops.noDoubledType);
        if (noDoubledRole && Number.isFinite(fsf.drops.noDoubledCount) && fsf.drops.noDoubledCount <= 0) {
            // A non-positive limit makes this type permanently undroppable,
            // even if captures-to-hand keeps it in the engine pocket.
            roles.delete(noDoubledRole);
        }
...
function cataloguedNeededPieceIdentities(meta: CataloguedVariantClientDocument, letters: Set<cg.Letter>): Set<string> {
    if (!meta.fsfVariantInfo) {
        return pieceIdentitiesForLetters(letters, cataloguedPieceIdentityContextKeys(meta));
    }
    const byRole = cataloguedFsfPieceByRole(meta);
    const promotedTypes = meta.fsfVariantInfo?.promotion.promotedPieceTypes ?? {};
    const identities = new Set<string>();
...
export function cataloguedCompatibleBoardFamily(
    ...
    const width = meta.fsfVariantInfo?.board.width ?? meta.width;
    const height = meta.fsfVariantInfo?.board.height ?? meta.height;
    if (
        meta.boardFamilyOverride &&
        BOARD_FAMILIES[meta.boardFamilyOverride] &&
        boardFamilyMatchesDimensions(meta.boardFamilyOverride, width, height)
    ) {
        return meta.boardFamilyOverride;
    }

    const baseVariant = meta.baseVariant ? VARIANTS[meta.baseVariant] : undefined;
    if (baseVariant && boardFamilyMatchesDimensions(baseVariant.boardFamily, width, height)) {
...
export function registerCataloguedVariant(meta: CataloguedVariantClientDocument): void {
    ...
    const boardFamily = cataloguedCompatibleBoardFamily(meta) ?? ensureCataloguedBoardFamily(width, height);
