...
export function updateCount(fen: string, whiteContainer: VNode | Element, blackContainer: VNode | Element) {
    ...
    if (countingLimit !== 0 && countingPly !== 0) {
        if (countingSide === 'w') whiteContainer = patch(whiteContainer, h('div#misc-infow', countingStr));
        else blackContainer = patch(blackContainer, h('div#misc-infob', countingStr));
    }
...
export function updatePoint(variant: Variant, fen: string, wContainer: VNode | Element, bContainer: VNode | Element) {
    ...
    wContainer = patch(
        ...
        h(
            'div#misc-infow',
            { class: { 'text-color-red': wcolor === 'Red', 'text-color-blue': wcolor === 'Blue' } },
            wPoint,
        ),
    );
    bContainer = patch(
        ...
        h(
            'div#misc-infob',
            { class: { 'text-color-red': bcolor === 'Red', 'text-color-blue': bcolor === 'Blue' } },
            bPoint,
        ),
    );
