        ...
        h('button.layer-2-category l3v', [
            ...
            h('ul.l3links-cont', [
                h(
                    'li.l3links',
                    {
                        ...
                        on: { click: () => lobbyCtrl.createGame(variantName) },
                    },
                    createModeStr('createGame'),
                ),
                h(
                    'li.l3links',
                    {
                        class: { icon: true, 'icon-crossedswords': true },
                        on: { click: () => lobbyCtrl.playFriend(variantName) },
                    },
                    createModeStr('playFriend'),
                ),
                h(
                    'li.l3links',
                    { class: { icon: true, 'icon-bot': true }, on: { click: () => lobbyCtrl.playAI(variantName) } },
            ...
            h(
                'h5#chessl3back',
                {
                    ...
                    on: { click: () => leve2func(lobbyCtrl, container3Id, lobbyCtrl.gameCategory === 'all') },
                },
                _('Go Back'),
            ),
        ]),
        h('button.layer-2-category l3img', [variantBoard(variant, variant.startFen)]),
        h('button.layer-2-category l3t', [
            h('p.variant-extra-info', chess960 ? chess960Tooltip(variant.name) : variant.tooltip),
            h(
                'a.variant-extra-info',
                {
...
function proTip(variant: string, chess960: boolean) {
    switch (variant) {
        ...
        case 'placement':
            return _(
                'Castling is only possible if the king and rook are dropped to their usual places like in standard Chess.',
            );
        case 'duck':
            return _('Quack.');
        case 'alice':
