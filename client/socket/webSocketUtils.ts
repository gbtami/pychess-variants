...
export function toggleSocketCssOnOpen() {
    ...
    if (wasOffline) {
        // dont trigger animation on first connect
...
export function toggleSocketCssOnReconnect(reconnectTimeoutMs: number) {
    ...
    document
        .getElementById('reconnecting-ts')!
        .setAttribute('timestamp', new Date(new Date().getTime() + reconnectTimeoutMs).toISOString());
}
...
export function toggleSocketCssOnCreateWebSocket() {
    ...
    const wasOffline = clearSocketCss();
    if (wasOffline) {
        // on first connect we dont do notification popups
