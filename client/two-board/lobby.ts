...
function seekUserLink(username: string) {
...
export function seekViewBughouse(ctrl: LobbyController, seek: Seek) {
    ...
    return h('tr', { on: { click: () => onClickSeekBughouse(ctrl, seek) } }, [
        h('td.bugseek1', [
            ...
            h('div', 'A'),
            colorIconBug(seek.color, 'player1'),
            h('div', 'B'),
            colorIconBug(seek.color, 'bugPlayer1'),
            h('div.bugseekvs', 'vs.'),
            h('div.bugseekteam2', h('div', '+')),
            h('div', 'A'),
            colorIconBug(seek.color, 'player2'),
            h('div', 'B'),
            colorIconBug(seek.color, 'bugPlayer2'),
        ]),
        h(
            'td',
            h('div.bugseek', [
                h('div', [ctrl.seekTitle(seek), seekUserLink(seek.user)]),
                h('div', [
                    ctrl.challengeIcon(seek),
                    ctrl.seekTitle(seek),
                    seek.bugPlayer1 === ''
                        ? [seek.player2, seek.bugPlayer2].includes(ctrl.username)
                            ? h('div.bugwaiting', 'Waiting...')
                            : h(
                                  'button.bug-join-button',
                                  { on: { click: (e: Event) => bugJoinSeek(ctrl, e, seek, 'bugPlayer1') } },
                                  _('Join'),
                              )
                        : seekUserLink(seek.bugPlayer1),
                ]),
                h('div.bugseekvs1', ''),
                h('div', [
                    ctrl.challengeIcon(seek),
                    ctrl.seekTitle(seek),
                    seek.player2 === ''
                        ? [seek.user, seek.bugPlayer1].includes(ctrl.username)
                            ? h('div.bugwaiting', 'Waiting...')
                            : h(
                                  'button.bug-join-button',
                                  { on: { click: (e: Event) => bugJoinSeek(ctrl, e, seek, 'player2') } },
                                  _('Join'),
                              )
                        : seekUserLink(seek.player2),
                ]),
                h('div', [
                    ctrl.challengeIcon(seek),
                    ctrl.seekTitle(seek),
                    seek.bugPlayer2 === ''
                        ? [seek.user, seek.bugPlayer1].includes(ctrl.username)
                            ...
                            : h(
                                  'button.bug-join-button',
                                  { on: { click: (e: Event) => bugJoinSeek(ctrl, e, seek, 'bugPlayer2') } },
                                  _('Join'),
                              )
                        : seekUserLink(seek.bugPlayer2),
                ]),
            ]),
        ),
        h('td', seek.rating),
        ...
        h('td', { class: { tooltip: seek.fen !== '' } }, [ctrl.tooltip(seek, variant), ctrl.mode(seek)]),
    ]);
}
...
function colorIconBug(color: string, slot: string) {
    return h('i-side.icon', {
        class: {
            ...
            'icon-white':
                (color === 'w' && ['player1', 'bugPlayer2'].includes(slot)) ||
                (color === 'b' && ['player2', 'bugPlayer1'].includes(slot)),
            'icon-black':
                (color === 'b' && ['player1', 'bugPlayer2'].includes(slot)) ||
                (color === 'w' && ['player2', 'bugPlayer1'].includes(slot)),
        },
    });
}
