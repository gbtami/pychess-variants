...
INDEX_SPECS = (
    ("d_id_desc", [("d", -1), ("_id", -1)]),
    ("v_d_id_desc", [("v", 1), ("d", -1), ("_id", -1)]),
    ("us_d_desc", [("us", 1), ("d", -1)]),
    ("us_s_d_desc", [("us", 1), ("s", 1), ("d", -1)]),
...
def _pattern_from_doc(index_doc: dict[str, Any]) -> tuple[tuple[str, int | str], ...]:
    # Mongo returns key as an ordered mapping.
    key_doc = index_doc.get("key", {})
    return tuple((str(k), int(v) if isinstance(v, int) else str(v)) for k, v in key_doc.items())
...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Create the non-essential compound game indexes outside server startup. "
    ...
    try:
        ...
        by_pattern: dict[tuple[tuple[str, int | str], ...], str] = {}
        for doc in existing:
            name = doc.get("name")
        ...
        if args.json_out:
            ...
            await asyncio.to_thread(_write_json_summary, args.json_out, payload)
