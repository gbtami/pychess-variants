...
- [x] **F1 — Full interaction review.** Run the acceptance matrix below for every
  ...
  **Done:** the cross-mode review keeps Normal analysis as the unrestricted baseline,
  confirms conceal/lesson/practice disclosure and local-attempt guards, and keeps
  compact embeds lightweight like lila's Study embed. Normal chapters retain the
  ordinary embedded viewer; non-Normal chapters are locked to a root-position preview
  and never instantiate lesson/practice playback or browser-engine work in the iframe.
  Interactive lessons show a centered **Start** link to the full Study; Practice and
  Hide-next-moves use the same safe non-interactive preview with an **Open study** link.
  The full Study playback panel uses theme variables, wraps at narrow widths and
  preserves visible keyboard focus; labelled regions provide atomic polite status
  updates and engine/script busy state. PGN export/import round-trips all chapter modes,
