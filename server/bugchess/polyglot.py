...
class MemoryMappedReader:
    ...
    def __init__(self, filename: PathLike) -> None:
        self.fd = os.open(
            filename, os.O_RDONLY | os.O_BINARY if hasattr(os, "O_BINARY") else os.O_RDONLY
        )
    ...
    def __contains__(self, entry: Entry) -> bool:
        return any(
            current == entry for current in self.find_all(entry.key, minimum_weight=entry.weight)
        )
    ...
    def find(
        ...
        *minimum_weight* ``0`` to select all entries.
        ...
        try:
            return max(
                self.find_all(board, minimum_weight=minimum_weight, exclude_moves=exclude_moves),
