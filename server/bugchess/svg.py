...
def _svg(viewbox: int | tuple[int, int] | None, size: int | tuple[int, int] | None) -> ET.Element:
...
def piece(piece: bugchess.Piece, size: int | None = None) -> str:
    """
...
def bughouse_boards(
    ...
    squaresL: bugchess.IntoSquareSet | None = None,
    squaresR: bugchess.IntoSquareSet | None = None,
    flipped: bool = False,
    coordinates: bool = True,
    lastmoveL: bugchess.Move | None = None,
    lastmoveR: bugchess.Move | None = None,
    checkL: bugchess.Square | None = None,
    checkR: bugchess.Square | None = None,
    arrowsL: Iterable[Arrow | tuple[bugchess.Square, bugchess.Square]] = (),
    arrowsR: Iterable[Arrow | tuple[bugchess.Square, bugchess.Square]] = (),
    size: int | None = None,
    style: str | None = None,
    player_text: Sequence[Sequence[str]] | None = None,
    ...
    svg_boards = [
        ET.SubElement(svg, "g"),
        ET.SubElement(svg, "g", {"transform": f"translate({single_board_width}, 0)"}),
    ]
    ...
    for i, s in enumerate(svg_boards):
        svg_content = ET.SubElement(s, "g", {"transform": f"translate(0, {height_offset})"})
...
def board(
    board: bugchess.BaseBoard | None = None,
    *,
    squares: bugchess.IntoSquareSet | None = None,
    flipped: bool = False,
    coordinates: bool = True,
    lastmove: bugchess.Move | None = None,
    check: bugchess.Square | None = None,
    arrows: Iterable[Arrow | tuple[bugchess.Square, bugchess.Square]] = (),
    size: int | None = None,
    style: str | None = None,
    base_svg: ET.Element | None = None,
    ...
    if isinstance(board, bugchess.variant.CrazyhouseBoard):
        svg_board = ET.SubElement(svg, "g", {"transform": f"translate(0, {2 * SQUARE_SIZE})"})
        svg_pocket_top = ET.SubElement(svg, "g", {"transform": f"translate({margin}, 0)"})
        svg_pocket_bottom = ET.SubElement(
            svg,
            "g",
            {"transform": f"translate({margin}, {10 * SQUARE_SIZE + 2 * margin})"},
        )
        ...
        for s, p in zip(
            ...
            for i, piece_type in enumerate(bugchess.PIECE_TYPES[:-1]):
                ...
                ET.SubElement(
                    ...
                    "use",
                    {
                        "xlink:href": f"#{bugchess.COLOR_NAMES[p.color]}-{bugchess.PIECE_NAMES[piece_type]}",
                        "transform": f"translate({i * SQUARE_SIZE:d}, {0:d})",
                    },
                )
                s.append(
                    _text(
        ...
        if board is not None:
            ...
            if piece:
                ET.SubElement(
                    ...
                    "use",
                    {
                        "xlink:href": f"#{bugchess.COLOR_NAMES[piece.color]}-{bugchess.PIECE_NAMES[piece.piece_type]}",
...
def pocket(
    pocket: variant.CrazyhousePocket, numbers_top: bool = False, width: int | None = None
) -> str:
    """
    ...
    for i, piece_type in enumerate(bugchess.PIECE_TYPES[:-1]):
        ...
        ET.SubElement(
            ...
            "use",
            {
                "xlink:href": f"#{bugchess.COLOR_NAMES[pocket.color]}-{bugchess.PIECE_NAMES[piece_type]}",
                "transform": f"translate({i * SQUARE_SIZE:d}, {pieces_y_pos:d})",
            },
        )
        svg.append(
            _text(
